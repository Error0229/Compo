(* PrintTypedAst.ml *)
open Ast
(* Assuming Ast module contains the definitions of texpr, tstmt, etc. *)
...
let rec print_texpr fmt = function
  | TEcst c -> Format.fprintf fmt "TEcst(%s)" (string_of_constant c)
  | TEvar v -> Format.fprintf fmt "TEvar(%s)" v.v_name
  | TEbinop (op, e1, e2) ->
    Format.fprintf
      fmt
      "TEbinop(%s, %a, %a)"
  ...
  | TElist exprs -> Format.fprintf fmt "TElist([%a])" print_texpr_list exprs
  | TErange e -> Format.fprintf fmt "TErange(%a)" print_texpr e
  | TEget (e1, e2) ->
...
let rec print_tstmt fmt = function
  | TSif (e, s1, s2) ->
    Format.fprintf
      ...
      print_tstmt
      s2
  | TSreturn e -> Format.fprintf fmt "TSreturn(%a)" print_texpr e
  | TSassign (v, e) ->
    Format.fprintf fmt "TSassign(%s, %a)" v.v_name print_texpr e
  | TSprint e -> Format.fprintf fmt "TSprint(%a)" print_texpr e
  | TSblock stmts -> Format.fprintf fmt "TSblock([%a])" print_tstmt_list stmts
  | TSfor (v, e, s) ->
    Format.fprintf fmt "TSfor(%s, %a, %a)" v.v_name print_texpr e print_tstmt s
  | TSeval e -> Format.fprintf fmt "TSeval(%a)" print_texpr e
  | TSset (e1, e2, e3) ->
    Format.fprintf
      fmt
      "TSset(%a, %a, %a)"
